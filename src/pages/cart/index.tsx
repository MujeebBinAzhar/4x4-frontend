...
export const getServerSideProps: GetServerSideProps = async ({ req }) => {
    ...
    const seo = await shopApi.getPageSeo('cart');
                            ...
                            {item.options.length > 0 && (
                                <ul className="cart-table__options">
                                    {item.options.map((option, optionIndex) => (
                                        <li key={optionIndex}>{`${option.name}: ${option.value}`}</li>
                                    ))}
                                </ul>
                            )}
                        </td>
    ...
    return (
        <React.Fragment>
            ...
            <div className="block">
                <div className="container">
                    <div className="cart">
                        <div className="cart__table cart-table">{table}</div>
                        <div className="cart__totals">{totals}</div>
                    </div>
                </div>
            </div>
