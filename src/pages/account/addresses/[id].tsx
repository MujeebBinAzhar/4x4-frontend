...
function Page() {
    ...
    const user = useUser();
    const addressId =
        router.query.id === 'new' || typeof router.query.id !== 'string' ? null : parseFloat(router.query.id);
    ...
    const [submit, submitInProgress] = useAsyncAction(
        (data: IForm) => {
            ...
            if (addressId) {
                ...
                saveMethod = accountApi.addAddress(addressData, user?._id || '');
            }
    ...
    useEffect(() => {
        ...
        (async () => {
            const [address, defaultAddress] = await Promise.all([
                ...
                accountApi.getDefaultAddress(user?._id || ''),
            ]);
