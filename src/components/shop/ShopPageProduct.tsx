...
import StockWarningMessage from "~/components/shared/StockWarningMessage";
...
function ShopPageProduct(props: Props) {
  ...
  const relatedProducts = useDeferredData(
    () => shopApi.getProductsList({ page: 1, limit: 10 }),
    {
      items: [],
      sort: "",
      navigation: {
        ...
        <React.Fragment>
          <div className="product__actions-item product__actions-item--quantity">
            <Controller
              name="quantity"
              rules={{ required: true }}
              render={({ field: { ref: fieldRef, ...fieldProps } }: any) => (
                <InputNumber
                  size="lg"
                  min={1}
                  {...fieldProps}
              ...
              {relatedProducts?.data?.items.length > 0 && (
                <React.Fragment>
                  ...
                  <BlockProductsCarousel
                    ...
                    products={relatedProducts?.data?.items}
