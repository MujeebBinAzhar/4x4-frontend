            ...
            product?.type?.attributeGroups
                ?.map((group) => ({
                    ...
                    attributes: group?.attributes
                        .map((attribute) => product.attributes.find((x) => x.slug === attribute) || null)
                        .filter((x) => x !== null),
                }))
        ...
        () => [
            {
                ...
                content: <div className="typography" dangerouslySetInnerHTML={{ __html: product.description }} />,
            },
            // {
                ...
                {tabs.map((tab, index) => (
                    ...
                    >
                        <AppLink href={{ href: { hash: tab.id } }} onClick={() => setActiveTab(tab.id)}>
                            {tab.title}
                            {tab.showCounter && <span className="product-tabs__item-counter">{tab.counter}</span>}
                        </AppLink>
                    </li>
                ))}
            </ul>
